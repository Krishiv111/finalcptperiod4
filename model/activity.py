...
# Define the map class to manage actions in 'maps' table,  with a relationship to 'users' table
class map(db.Model):
  __tablename__ = 'maps'
...
class User(db.Model):
  ...
  maps = db.relationship("map", cascade='all, delete', backref='users', lazy=True)
  ...
  def read(self):
      return {
          ...
          "fun": self.fun,
          "maps": [map.read() for map in self.maps]
      }
  ...
  for user in users:
      try:
          ...
          for num in range(randrange(1, 4)):
              user.maps.append(map(id=user.id, address=user._address, coordinates=user._coordinates ,fun = user._fun))
          '''add user/map data to table'''
          user.create()
      except IntegrityError:
